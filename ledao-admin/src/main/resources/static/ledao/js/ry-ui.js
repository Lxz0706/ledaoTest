...
(function ($) {
    $.extend({
        ...
        table: {
            ...
            tooltip: function (value, length, target, str) {
                ...
                if (_value.length > _length) {
                    ...
                    actions.push($.common.sprintf('<a href="javascript:void(0)" class="tooltip-show" data-toggle="tooltip" data-target="%s" title="%s">%s</a>', _target, _value, _text));
                    return actions.join('');
                } else {
                    _text = _value;
                    return _text;
        ...
        modal: {
            ...
            closeOpen: function (title, url, width, height, callback) {
                //如果是移动端，就使用自适应大小弹窗
                if ($.common.isMobile()) {
                    width = 'auto';
                    height = 'auto';
        ...
        operate: {
            ...
            // 添加档案申请
            addApplyIn: function (id) {
                table.set();
                $.modal.closeOpen("添加" + table.options.modalName, $.operate.addUrl(id));
            },
            // 添加信息
            add: function (id) {
                table.set();
            ...
            successTabCallback: function (result) {
                if (result.code == web_status.SUCCESS) {
                    ...
                    var $contentWindow = $('.LeDao_iframe[data-id="' + currentId + '"]', topWindow)[0].contentWindow;
                    $.modal.close();
